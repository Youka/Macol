...
function create_confi(subs)
	...
	for _, style in ipairs(collect_styles(subs)) do
		table.insert(conf[5].items,"Style: " .. style)
	end
	return conf
end
...
function change_tag(subs,index,config)
	...
	if a.text:find("{") and a.text:find("}") then
		if config.margin == "{..." then
			a.text = a.text:gsub("{", string.format("{%s",config.txt), config.check and 1 or nil)
		else
			a.text = a.text:gsub("}", string.format("%s}",config.txt), config.check and 1 or nil)
		end
	else
...
function add_tags(subs,sel,config)
	if config.chosen == "Selected Lines" then
		...
		local style = config.chosen:sub(8)
		for i=1, #subs do
			if subs[i].style == style then change_tag(subs,i,config) end
		end
	end
end
